...
public class Banner2 extends FrameLayout implements OnPageChangeListener {
    ...
    private boolean needTurn = true;//是否轮播
    ...
    public Banner2 setBanner2Titles(List<String> titles) {
        this.titles = titles;
        return this;
    }
    ...
    private void setImageList(List<?> imagesUrl) {
        ...
        initImages();
        for (int i = 0; i < (needTurn ? count + 2 : count); i++) {
            View imageView = null;
            if (imageLoader != null) {
    ...
    private void setData() {
        currentItem = 1;
        if (adapter == null) {
            adapter = new Banner2.BannerPagerAdapter();
            viewPager.addOnPageChangeListener(this);
        }
        viewPager.setAdapter(adapter);
        viewPager.setFocusable(true);
        if (needTurn) viewPager.setCurrentItem(1);
